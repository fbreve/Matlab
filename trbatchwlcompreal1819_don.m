rep = 20; % numero de repetições (conjuntos diferentes)
prep = 20; % repetiçoes dos algoritmos de partículas (em cada conjunto)
amount = (40/150); % quantidade de elementos pré-rotulados
qtalg = 7;
amwrlabmax = 21;
disttype = 'euclidean';
pdet=0.5; deltav=0.1;
...
for i=1:1:amwrlabmax
    ...
    for l=1:rep
        ...
        fitfunstrwalk = @(x)fitstrwalk8kk(x,X,slabel,label,disttype,1);
        ...
        parfor j=1:prep
            ...
            disp(sprintf('Particles (8k) - %0.2f RE - Rep %3.0f/%3.0f - Erro: %0.4f - K: %2.0f - Pgrd: %0.2f DeltaV: %0.2f',amwrlab,j,prep,part(j),k, pdet, deltav))
        end
        iter_acc(l,4) = mean(part);
        disp(sprintf('Particles (8k)- %0.2f RE - Erro: %0.4f - K: %2.0f - Pgrd: %0.2f DeltaV: %0.2f',amwrlab,iter_acc(l,4),k, pdet, deltav))

        disp(sprintf('Rodando Algoritmo Genético para %0.2f rótulos errados - Rep. %2.0f/%2.0f - Particles (11)',amwrlab,l,rep))       
        fitfunstrwalk = @(x)fitstrwalk11(x,X,slabel,label,disttype,1);
        options = gaoptimset('Display','iter','UseParallel','always','FitnessLimit',0,'StallGenLimit',2,'Generations',5,'InitialPopulation',[1;2;4;6;8;12;15;20;30;40;50;75;100;125;150;175;200;250;300]);
        [gaout,fval] = ga(fitfunstrwalk,1,[],[],[],[],1,300,[],options);     
        k = round(gaout);
        ...
        parfor j=1:prep
            ...
            part2(j) = 1 - stmwevalk(label,slabel,owner,1);
            disp(sprintf('Particles (11) - %0.2f RE - Rep %3.0f/%3.0f - Erro: %0.4f - K: %2.0f - Pgrd: %0.2f DeltaV: %0.2f',amwrlab,j,prep,part2(j),k, pdet, deltav))
        end
        iter_acc(l,5) = mean(part2);
        disp(sprintf('Particles (11) - %0.2f RE - Erro: %0.4f - K: %2.0f - Pgrd: %0.2f DeltaV: %0.2f',amwrlab,iter_acc(l,5),k, pdet, deltav))   
              
        disp(sprintf('Rodando Algoritmo Genético para %0.2f rótulos errados - Rep. %2.0f/%2.0f - Particles (18)',amwrlab,l,rep))       
        fitfunstrwalk = @(x)fitstrwalk18(x,X,slabel,label,disttype,1);
        options = gaoptimset('Display','iter','UseParallel','always','FitnessLimit',0,'StallGenLimit',2,'Generations',5,'InitialPopulation',[1;2;4;6;8;12;15;20;30;40;50;75;100;125;150;175;200;250;300]);
        [gaout,fval] = ga(fitfunstrwalk,1,[],[],[],[],1,300,[],options);     
        k = round(gaout);
        disp(sprintf('Particles (18) - %0.2f RE - Erro: %0.4f - K: %2.0f',amwrlab,fval,k))
        part3 = zeros(prep,1);
        parfor j=1:prep
            [owner, pot, distnode] = strwalk18(X, slabel, k, disttype, pdet, deltav);
            part3(j) = 1 - stmwevalk(label,slabel,owner,1);
            disp(sprintf('Particles (18) - %0.2f RE - Rep %3.0f/%3.0f - Erro: %0.4f - K: %2.0f - Pgrd: %0.2f DeltaV: %0.2f',amwrlab,j,prep,part3(j),k, pdet, deltav))
        end
        iter_acc(l,6) = mean(part3);
        disp(sprintf('Particles (18) - %0.2f RE - Erro: %0.4f - K: %2.0f - Pgrd: %0.2f DeltaV: %0.2f',amwrlab,iter_acc(l,6),k, pdet, deltav))

        disp(sprintf('Rodando Algoritmo Genético para %0.2f rótulos errados - Rep. %2.0f/%2.0f - Particles (19)',amwrlab,l,rep))       
        fitfunstrwalk = @(x)fitstrwalk19(x,X,slabel,label,disttype,1);
        options = gaoptimset('Display','iter','UseParallel','always','FitnessLimit',0,'StallGenLimit',2,'Generations',5,'InitialPopulation',[1;2;4;6;8;12;15;20;30;40;50;75;100;125;150;175;200;250;300]);
        [gaout,fval] = ga(fitfunstrwalk,1,[],[],[],[],1,300,[],options);     
        k = round(gaout);
        disp(sprintf('Particles (19) - %0.2f RE - Erro: %0.4f - K: %2.0f',amwrlab,fval,k))
        part4 = zeros(prep,1);
        parfor j=1:prep
            [owner, pot, distnode] = strwalk19(X, slabel, k, disttype, pdet, deltav);
            part4(j) = 1 - stmwevalk(label,slabel,owner,1);
            disp(sprintf('Particles (19) - %0.2f RE - Rep %3.0f/%3.0f - Erro: %0.4f - K: %2.0f - Pgrd: %0.2f DeltaV: %0.2f',amwrlab,j,prep,part4(j),k, pdet, deltav))
        end
        iter_acc(l,7) = mean(part4);
        disp(sprintf('Particles (19) - %0.2f RE - Erro: %0.4f - K: %2.0f - Pgrd: %0.2f DeltaV: %0.2f',amwrlab,iter_acc(l,7),k, pdet, deltav))        
        
        disp(sprintf('COMP: Perc: %0.4f - Rep. %2.0f/%2.0f - Err: CO: %0.4f  LP: %0.4f  LNP: %0.4f  Part8k: %0.4f  Part11: %0.4f  Part18: %0.4f  Part19: %0.4f',amwrlab,l,rep,iter_acc(l,1),iter_acc(l,2),iter_acc(l,3),iter_acc(l,4),iter_acc(l,5),iter_acc(l,6),iter_acc(l,7)))
...
%     disp(sprintf('FINAL: Perc: %0.4f  Kap: CO: %0.4f  LP: %0.4f  LNP: %0.4f  P1: %0.4f  P2: %0.4f',amwrlab,tab_kap(i,1),tab_kap(i,2),tab_kap(i,3),tab_kap(i,4),tab_kap(i,5)))
    save tabs_wlcompreal_don tab_acc tab_std_acc tab_min_acc tab_max_acc;
    disp(sprintf('FINAL: Perc: %0.4f  Err: CO: %0.4f  LP: %0.4f  LNP: %0.4f  Part8k: %0.4f  Part11: %0.4f  Part18: %0.4f  Part19: %0.4f',amwrlab,tab_acc(i,1),tab_acc(i,2),tab_acc(i,3),tab_acc(i,4),tab_acc(i,5),tab_acc(i,6),tab_acc(i,7)))
end
