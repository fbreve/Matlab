...
% Usage: [owner, pot] = mostrwalk8kef3(X, slabel, k, pdet, deltav, deltap, dexp, nclass, iter)
function [owner, pot] = mostrwalk8kef3(X, slabel, k, pdet, deltav, deltap, dexp, nclass, iter)
    ...
    for i=1:iter
        ...
        for j=1:npart          
            ...
            if rndtb(j)<pdet
                ...
                cn = N{partpos(j)}(find(prob>=(roulettepick(j)*prob(end)),1,'first'));
            else
                cn = N{partpos(j)}(ceil(roulettepick(j)*size(N{partpos(j)},2)));
            end
            ...
            if slabel(cn)==0
                ...
                deltapotpart = pot(cn,:) - max(potmin,pot(cn,:) - potpart(j)*(deltav/(nclass-1)));
                pot(cn,:) = pot(cn,:) - deltapotpart;
                pot(cn,partclass(j)) = pot(cn,partclass(j)) + sum(deltapotpart);
            end
            % atribui novo potencial para partícula
            potpart(j) = potpart(j) + (pot(cn,partclass(j))-potpart(j))*deltap;
                      
            % se distância do nó alvo maior que distância do nó atual + 1
            if distnode(partpos(j),j)+1<distnode(cn,j)
                % atualizar distância do nó alvo
                distnode(cn,j) = distnode(partpos(j),j)+1;
            end
            
            % se não houve choque
            if pot(cn,partclass(j))>=max(pot(cn,:))
                % muda para nó alvo
                partpos(j) = cn;
            end
        end
