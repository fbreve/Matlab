% Semi-Supervised Territory Mark Walk v.10
...
% Calcula kernel (protótipo) de cada cluster (v.10)
% Usage: [owner, owner2, pot, owndeg, distnode, cmeans] = strwalk10(X, slabel, sigma, pdet, deltav, deltap, dexp, iter, nclass)
function [owner, owner2, pot, owndeg, distnode, cmeans] = strwalk10(X, slabel, sigma, pdet, deltav, deltap, dexp, iter, nclass)
    ...
    % rótulos pelos potenciais instantâneos
    [~,owner] = max(pot,[],2);
    % kernels (protótipos)
    ...
    % rótulos pelos potenciais acumulados
    [~,owner2] = max(owndeg,[],2);       
end
