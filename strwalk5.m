% Semi-Supervised Territory Mark Walk v.5
...
% Usage: [owner, pot, distnode] = strwalk5(X, slabel, nclass, iter, pdet, deltav, deltap, sigma)
function [owner, pot, distnode] = strwalk5(X, slabel, nclass, iter, pdet, deltav, deltap, sigma)
    ...
    for i=1:iter
        ...
        for j=1:npart
            ...
            if random('unif',0,1)<pdet
                ...
                prob = graph(partpos(j),:).*(1./(1+distnode(:,j)).^sigma)';  
            else
                % regra de probabilidade
