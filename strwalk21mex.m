...
function [owner, slabel, pot, owndeg, distnode] = strwalk21mex(X, label, labp, k, disttype, beta, valpha, pdet, deltav, deltap, dexp, nclass)
    ...
    % eliminando a distância pnara o próprio elemento
    ...
    while 1
        ...
        if cnpart < npart
            ...
            % calcula impacto
%             if (cnpart>nclass) % se não é a primeira passada
...
%             end
           ...
           % end
            ...
            %disp(sprintf('Querytype: %1.0f | Nó Escolhido: %1.0f | Valor: %0.4f',querytype,ind,tot))
            ...
            cnpart = cnpart + 1;                      
            % rotulando nó escolhido
            slabel(ind) = label(ind);
            % ajustando potenciais do nó escolhido
            pot(ind,:)=0;
            pot(ind,label(ind))=1;
