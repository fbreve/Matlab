% Semi-Supervised Territory Mark Walk v.35
...
% Trabalha na primeira fase com imagem redimensionada para metade do tamanho
% original. Traz de volta a tabela de distâncias individual. Inclui ExR, ExB, e ExG (v.35)
%
...
function [owner, pot, owndeg] = strwalk35teste(img, imgslab, fw, k, disttype, valpha, pgrd, deltav, deltap, dexp, maxiter)
...
if k>0
    ...
    %disp('Parte 1: Obtendo atributos...')
    ...
    [~, owndeg] = strwalk8kloop(maxiter, npart, nclass, stopmax, pgrd, dexp, deltav, deltap, potmin, partnode, partclass, potpart, slabelval, knns, distnode, KNN, potval, owndeg);
...
X(:,3:5) = double(squeeze(reshape(img,dim(1)*dim(2),1,3)))/255;
...
g = imfilter(rgb2hsv(img), h, 'replicate'); % adicionado replicate para que bordas não fiquem diferentes)
