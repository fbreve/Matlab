		   ...
		   )
{
    ...
    for(int i=0; i<maxiter; i++)
    {
        for(int j=0; j<npart; j++)
        {
            ...
            int greedymov;
            //printf("%0.4f\n",r);            
            if (r < (1-potpart[j])) // movimento guloso            
            //if (r < 0.5) // movimento guloso           
            {
                greedymov=1;
...
{ 
    ...
    double dexp, deltav, deltap, potmin;  // escalares double
    ...
    strwalk31loop(maxiter,npart,nclass,stopmax,dexp,deltav,deltap,potmin,partpos,partclass,potpart,slabel,nsize,distnode,nlist,pot,owndeg,qtnode,neibmax);
