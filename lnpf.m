...
% versão f (usa variáveis tipo single para utilizar menos memória)
function owner = lnpf(X,slabel,k,alpha,nclass,iter)
...
%disp('Montando rede')
W = zeros(qtnode,'single'); % inicializando matriz de adjacência
parfor i=1:qtnode
...
%    w = w ./ sum(w); % normalizar pesos encontrados
   ...
   w = quadprog(double(G),ones(1,k),[],[],ones(1,k),1,zeros(k,1),ones(k,1),[],options);
   % fim solução
   wi = zeros(1,qtnode); 
   wi(IX(:,i)) = w;
   W(i,:) = wi;    
