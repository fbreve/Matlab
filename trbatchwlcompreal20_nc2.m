...
for i=2:1:amwrlabmax
    ...
    for l=1:rep
        ...
        options = gaoptimset('Display','iter','UseParallel','always','FitnessLimit',0,'StallGenLimit',2,'Generations',5,'InitialPopulation',[0.001;0.01;0.05;0.1;0.2;0.35;0.5;0.75;1.0;2.0;3.5;5.0;7.5;10]);
        ...
        part2 = zeros(prep,1);
        parfor j=1:prep
            [owner, pot, owndeg] = strwalk8kmex(X, slabel, k, disttype, pdet, deltav);
        ...
        part3 = zeros(prep,1);
        parfor j=1:prep
            [owner, pot] = strwalk11mex(X, slabel, k, disttype, pdet, deltav);
        ...
        fitfunstrwalk = @(x)fitstrwalk20(x,X,slabel,label,disttype,1);
        options = gaoptimset('Display','iter','UseParallel','always','FitnessLimit',0,'StallGenLimit',2,'Generations',5,'InitialPopulation',[1;2;4;6;8;12;15;20;30;40;50;75;100;125;150;175;200;250;300]);
        [gaout,fval] = ga(fitfunstrwalk,1,[],[],[],[],1,300,[],options);     
        k = round(gaout);
        ...
        part4 = zeros(prep,1);
        parfor j=1:prep
            [owner, pot] = strwalk20mex(X, slabel, k, disttype, pdet, deltav);
...
%     disp(sprintf('FINAL: Perc: %0.4f  Kap: CO: %0.4f  LP: %0.4f  LNP: %0.4f  P1: %0.4f  P2: %0.4f',amwrlab,tab_kap(i,1),tab_kap(i,2),tab_kap(i,3),tab_kap(i,4),tab_kap(i,5)))
    save tabs_wlcompreal20_nc2 tab_acc tab_std_acc tab_min_acc tab_max_acc;
