...
#define nlist_IN        prhs[14]
#define ndist_IN        prhs[15]
		   ...
		   )
{
    ...
    double *distnode = malloc(sizeof(double)*qtnode*npart); // tabela de distâncias
    for(int i=0; i<qtnode*npart; i++)
        distnode[i] = qtnode-1;
    for(int i=0; i<npart; i++)
        distnode[(int) (qtnode*i + partpos[i]-1)] = 0;
    ...
    for(int i=0; i<maxiter; i++)
    {
        for(int j=0; j<npart; j++)
        {
            ...
            if (r < pgrd) // movimento guloso
            {
                ...
                for(int i2=0; i2< (int) nsize[(int) partpos[j]-1]; i2++)
                {
                    prob[i2] = DBL_MIN + 
                            ...
                            //(1 / pow(1+distnode[(int) ((partclass[j]-1) * qtnode + nlist[(int)(qtnode * i2 + partpos[j]-1)]-1)],dexp)) *
            ...
            //if (distnode[(int) ((partclass[j]-1) * qtnode + partpos[j]-1)] + ndist[(int) (nt*qtnode + partpos[j]-1)] < distnode[(int) ((partclass[j]-1)*qtnode + k-1)])                                            
                // atualizar distância do nó alvo
                //distnode[(int) ((partclass[j]-1)*qtnode + k-1)] = distnode[(int) ((partclass[j]-1)*qtnode + partpos[j]-1)] + ndist[(int) (nt*qtnode + partpos[j]-1)];    
            ...
            //if (fmax(distnode[(int) ((partclass[j]-1) * qtnode + partpos[j]-1)],ndist[(int) (nt*qtnode + partpos[j]-1)]) < distnode[(int) ((partclass[j]-1)*qtnode + k-1)])  
            //    distnode[(int) ((partclass[j]-1)*qtnode + k-1)] = fmax(distnode[(int) ((partclass[j]-1) * qtnode + partpos[j]-1)],ndist[(int) (nt*qtnode + partpos[j]-1)]);                
...
{ 
    ...
    strwalk22loop(maxiter,npart,nclass,stopmax,pgrd,dexp,deltav,deltap,potmin,partpos,partclass,potpart,slabel,nsize,nlist,ndist,pot,qtnode,neibmax);
