...
% Em vez de eliminar partícula, seleciona outra casa para ela (v.19)
% Usage: [owner, pot, owndeg, distnode] = strwalk19(X, slabel, k, disttype, pdet, deltav, nclass, iter)
function [owner, pot, owndeg, distnode] = strwalk19(X, slabel, k, disttype, pdet, deltav, nclass, iter)
    ...
    for chi=1:npart;
        ...
        [~,indnodencmaxowndeg] = max(owndeg(:,slabel(partnode(indpartminownowndeg))).*double(slabel==0));
