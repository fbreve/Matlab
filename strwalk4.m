% Semi-Supervised Territory Mark Walk v.4
% Derivado de strwalk3.m
% Conta distância de de nós para o nó pré-rotulado mais próximo (v.2)
% Utiliza uma partícula por nó pré-rotulado (v.3) 
% Valores finais baseados na média de potencial da vizinhança
% Usage: [owner, pot, distnode] = strwalk3(X, slabel, nclass, iter, pdet, deltav, deltap, sigma)
function [owner, owndeg, pot, distnode] = strwalk4(X, slabel, nclass, iter, pdet, deltav, deltap, sigma)
    ...
    for i=1:qtnode
        ...
        if slabel(i)~=0
            ...
            distnode(j,i)=0; % definindo distância do nó pré-rotulado para 0 na tabela de sua respectiva partícula
    ...
    %[nil,owner] = max(pot,[],2);
    owndeg = (graph * pot) ./ (sum(graph,2) * ones(1,nclass));
