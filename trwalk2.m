% Territory Mark Walk
% Usage: [owner, pot, logp, logn, lognc] = trwalk2(graph, npart, iter)
function [owner, pot, logp, logn, lognc] = trwalk2(graph, npart, iter)
    % constantes
    potmax = 1.000; % potencial máximo
    potmin = 0.005; % potencial mínimo
    deltap = 0.100; % controle de velocidade de aumento/decremento do potencial da partícula  
    deltav = 0.005; % controle de velocidade de aumento/decremento do potencial do vértice
    ...
    for i=1:iter
        ...
        for j=1:npart
            ...
            prob = graph(partpos(j),:)' .* (0.5 + (0.5 - (owner~=j & owner~=0)) .* pot);
            ...
            % se nó é de outra partícula
                ...
                potpart(j) = potpart(j) - deltap * 0.1;
                pot(partpos(j)) = pot(partpos(j)) - deltav * potpart(j);
