...
function [owner, pot] = strwalk24(X, slabel, k, disttype, valpha, nclass, maxiter)
    ...
    for i=1:maxiter
        ...
        for j=1:npart
            ppj = partnode(j);
            newpot(ppj,:) = sum(pot(KNN(ppj,1:knns(ppj)),:) .* repmat(KNND(ppj,1:knns(ppj))',1,nclass)) ./ sum(KNND(ppj,1:knns(ppj)));
        end
        pot = newpot;
        if mod(i,10)==0
