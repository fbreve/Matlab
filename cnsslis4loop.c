    ...
    for(int i=0; i<maxiter; i++)
    {
        ...
        {            
            ...
            {           
                ...
                newpot[j] += potval[neib] * (k-ki);
                newpot[j + nnonlabeled] += potval[neib + qtnode] * (k-ki);
            }
            // dividindo os potenciais acumulados pela quantidade de vizinhos, para obter a média
            newpot[j] /= (k*(k+1)/2);
            newpot[j + nnonlabeled] /= (k*(k+1)/2);
        }
...
{    
    ...
    cnsslisloop(maxiter,nnonlabeled,indnonlabeled,stopmax,potval,k,nlist,qtnode);
