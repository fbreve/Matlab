...
% Usage: [owner, slabel, pot, owndeg, distnode] = strwalk21(X, label, labp, k, disttype, valpha, pdet, deltav, deltap, dexp, nclass)
function [owner, slabel, pot, owndeg, distnode] = strwalk21(X, label, labp, k, disttype, valpha, pdet, deltav, deltap, dexp, nclass)
            ...
            if mmpot>maxmmpot
                ...
                if stopcnt > stopmax                     
                    if cnpart < npart
                        ...
                        % medindo distância
                        mindist = min(distnode,[],2);
                        % escolhendo um nó baseado em ambíguidade ou
                        r = unifrnd(0,1);
                        querytype = r > beta;
                        if querytype == 0
                            [~,ind] = max(uncert);
                        ...
                        % distância                        
                        %[tot,ind] = max(alpha * uncert - (1-alpha) * (1./(1+mindist.^2)));                        
                        %disp(sprintf('r: %0.4f | Alpha: %0.4f | Uncert: %0.4f | MinDist: %0.4f | Total: %0.4f',r,alpha,uncert(ind),1/(1+mindist(ind)^2),tot))                        
                        % verifica quem é dono do nó escolhido
                        [~,chosenowner] = max(pot(ind,:));
                        disp(sprintf('QueryType: %1.0f | Beta: %0.4f | Uncert: %0.4f | MinDist: %3.0f | Hit: % 1.0f',querytype,beta,uncert(ind),mindist(ind),chosenowner ~= label(ind)))
                        % se houve acerto
                        if (querytype==0 && chosenowner ~= label(ind)) || (querytype==1 && chosenowner == label(ind))
                            beta = min(1,beta + 0.1 * (1-uncert(ind)));
                        else
                            beta = max(0,beta - 0.1 * (1-uncert(ind)));
                        end
