        ...
        {           
            ...
            for(int i2=0; i2<nsize[(int) partnode[j]-1]; i2++)
            {
                for(int i3=0; i3<nclass; i3++)
                    nc[i3] = nc[i3] + pot[(int) (i3*qtnode + nlist[(int) (i2*qtnode + partnode[j]-1)]-1)] * ndist[(int) (i2*qtnode + partnode[j]-1)];
                sumweight += ndist[(int) (i2*qtnode + partnode[j]-1)];
            }
            
            for(int i2=0; i2<nclass; i2++)
                newpot[(int) (i2*qtnode + partnode[j]-1)] = nc[i2] / sumweight;
...
{ 
    ...
    strwalk24loop(maxiter,npart,nclass,stopmax,partnode,slabel,nsize,nlist,ndist,pot,qtnode,neibmax);
