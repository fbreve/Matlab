% Semi-Supervised Territory Mark Walk v.15
...
% Select only the more far away node (v.17)
% Usage: [owner, slabel, pot, owndeg, distnode] = strwalk17(X, label, labp, k, disttype, pdet, deltav, deltap, dexp, nclass)
function [owner, slabel, pot, owndeg, distnode] = strwalk17(X, label, labp, k, disttype, pdet, deltav, deltap, dexp, nclass)
            ...
            if mmpot>maxmmpot
                ...
                if stopcnt > stopmax                     
                    if cnpart < npart
                        ...
                        % descobrindo qual é o nó mais distante
                        [~,ind] = max(min(distnode,[],2));
