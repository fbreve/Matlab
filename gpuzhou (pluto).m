...
% e Bernard Schölkopf
% Uso: owner = gpuzhou(X,slabel,nclass,iter,alpha,sigma)
...
W = exp(-squareform(pdist(X,'seuclidean').^2)/2*sigma^2); % primeiro passo (distancia euclidiana nao normalizada)
W = W - eye(qtnode);  % zerando diagonal
D = diag(sum(W,2));
DInv = GPUsingle(D^(-1/2));
