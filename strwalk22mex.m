...
% Partículas apenas em nós rotulados com conexão com nós não rotulados (v.22)
...
function [owner, pot] = strwalk22mex(img, imgslab, slabtype, valpha, pgrd, deltav, dexp, maxiter)
    ...
    % vetor para guardar se nó é ativo ou não
    ...
    % definindo posição inicial de cada partícula
    partpos = find(slabel~=0 & actnode~=0);    
    pot = strwalk22loop(maxiter, npart, nclass, stopmax, pgrd, dexp, deltav, 1, potmin, partpos, partclass, potpart, slabel, Nsize, Nlist, Ndist, pot);             
    [~,owner] = max(pot,[],2);
