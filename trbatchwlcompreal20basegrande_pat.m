...
%         disp(sprintf('LNP - %0.2f RE - Erro: %0.4f - K: %2.0f',amwrlab,fval,round(gaout)))
        ...
        fitfunstrwalk = @(x)fitstrwalk8kk(x,X,slabel,label,disttype,valpha_ga,1);
        options = gaoptimset('Display','iter','UseParallel','always','FitnessLimit',0,'StallGenLimit',2,'Generations',5);
        [gaout,fval] = ga(fitfunstrwalk,1,[],[],[],[],1,100,[],options);        
        k = round(gaout);
        ...
        fitfunstrwalk = @(x)fitstrwalk11(x,X,slabel,label,disttype,valpha_ga,1);
        options = gaoptimset('Display','iter','UseParallel','always','FitnessLimit',0,'StallGenLimit',2,'Generations',5);
        [gaout,fval] = ga(fitfunstrwalk,1,[],[],[],[],1,100,[],options);     
        k = round(gaout);
        ...
        fitfunstrwalk = @(x)fitstrwalk20(x,X,slabel,label,disttype,valpha_ga,1);
        options = gaoptimset('Display','iter','UseParallel','always','FitnessLimit',0,'StallGenLimit',2,'Generations',5);
        [gaout,fval] = ga(fitfunstrwalk,1,[],[],[],[],1,100,[],options);     
        k = round(gaout);
...
%     disp(sprintf('FINAL: Perc: %0.4f  Kap: CO: %0.4f  LP: %0.4f  LNP: %0.4f  P1: %0.4f  P2: %0.4f',amwrlab,tab_kap(i,1),tab_kap(i,2),tab_kap(i,3),tab_kap(i,4),tab_kap(i,5)))
    save tabs_wlcompreal20_pat tab_acc tab_std_acc tab_min_acc tab_max_acc;
