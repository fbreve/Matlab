...
for subset=1:1:subsetmax
    ...
    fitfunstrwalk = @(x)fitstrwalk13(x,X,slabel,label,'euclidean'); 
    ...
    disp(sprintf('Valores otimizados: K: %02.0f',k))
    parfor i=1:rep
        %[owner, pot, owndeg, distnode] = strwalk8k(X, slabel, k, pdet, deltav, 'euclidean'); 
        [owner, pot, owndeg, distnode] = strwalk13(X, slabel, k, 'euclidean'); 
    ...
    save tabs_sqlbook_don tab_par tab_err tab_std_err tab_min_err tab_max_err tab_err2 tab_std_err2 tab_min_err2 tab_max_err2;
end
