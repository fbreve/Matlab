% Semi-Supervised Territory Mark Walk v.8k (versão que roda na GPU)
...
% Usage: [owner, pot, owndeg, distnode] = gpustrwalk8ke(X, slabel, k, pdet, deltav, deltap, dexp, nclass, iter)
function [owner, pot, owndeg, distnode] = gpustrwalk8ke(X, slabel, k, pdet, deltav, deltap, dexp, nclass, iter)
    ...
    graph = W <= repmat(B(:,k+1),1,qtnode);  % conectando k-vizinhos mais próximos
    graph = graph | graph';
    graph = graph - eye(qtnode,GPUsingle);  % zerando diagonal do grafo
    %graph = X;
    % tabela de potenciais de nós
    pot = repmat(GPUsingle(potmax/nclass),qtnode,nclass);
    % definindo potencial da partícula
    potpart = repmat(GPUsingle(potmax),npart,1);
    % definindo tabela de distâncias dos nós
    distnode = repmat(GPUsingle(qtnode-1),qtnode,npart);
    ...
    owndeg = repmat(GPUsingle(realmin),qtnode,nclass);  % não podemos usar 0, porque nós não visitados dariam divisão por 0
    % variável para guardar máximo potencial mais alto médio
    maxmmpot = 0;
    for i=1:iter
        % para cada partícula
        rndtb = GPUsingle(unifrnd(0,1,npart,1));
        for j=1:npart
            % calculando probabilidade de exploração
            if rndtb(j)<pdet
                % regra de probabilidade
                %prob = graph(partpos(j),:).*(1./(alpha.^distnode(:,j)))';  
                prob = cumsum(single(graph(partpos(j),:).*(1./(1+distnode(:,j)).^dexp)'.* pot(:,partclass(j))'));
                movtype = 0;
            else
                % regra de probabilidade
                prob = cumsum(single(graph(partpos(j),:)));   %.*pot(:,j)';
                movtype = 1;
            end
            ...
            if slabel(k)==0
                ...
                deltapotpart = pot(k,:) - GPUsingle(max(potmin,single(pot(k,:) - potpart(j)*(deltav/(nclass-1)))));
            ...
            % se não houve choque
            if pot(k,partclass(j))>=max(single(pot(k,:)))
                % muda para nó alvo
                partpos(j) = k;
            end
        end
        if mod(i,ivconv)==0
            mmpot = mean(max(single(pot),[],2));
    ...
    [~,owner] = GPUsingle(max(single(pot,[],2)));
