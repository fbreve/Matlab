...
% fase intermediária com imagem em tamanho real (v7)
...
if (nargin < 5) || isempty(k2)
    k2 = 8; % quantidade de vizinhos mais próximos
end
if (nargin < 4) || isempty(k1)
    k1 = 8; % quantidade de vizinhos mais próximos
end
...
if k2>0
    if k1>0
        ...
        % se a primeira fase foi pulada, não há um conjunto pot criado,
        % então a chamada é igual a de primeira fase
...
% se duas fases anteriores não foram rodadas, precisamos criar pot
if (k1==0 && k2==0)
    pot = repmat(0.5,qtnode,nclass);    
elseif k2==0 % se apenas segunda fase não foi rodada, precisamos redimensionar pot
    pot = reshape(imresize(reshape(pot,rs_dim(1),rs_dim(2),2),[dim(1) dim(2)],'bilinear'),qtnode,2);
end
% se segunda fase não foi rodada, precisamos ajustar pot dos nós rotulados
if k2==0 
...
% PARTE 3
indefnodesb = max(pot,[],2)<0.99; % vetor onde 1 é nó indefinido e 0 é definido
...
function pot = lp(phase,qtnode,X,slabel,nodeval,k,disttype,valpha,maxiter,pot)
    ...
    if phase==1        
        ...
        % se estamos na segunda fase vamos tratar apenas os nós indecisos
        potval = pot(indval,:);
        indnonlabeled = uint32(find(slabelval==0 & max(potval,[],2)<0.99));
    end
    nnonlabeled = size(indnonlabeled,1); % quantidade de nós não rotulados   
