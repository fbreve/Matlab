...
function [owner, pot, owndeg] = strwalk37(img, imgslab, fw, k, sigma, disttype, valpha, pgrd, deltav, deltap, dexp, maxiter)
if (nargin < 12) || isempty(maxiter)
...
if k>0
    ...
    [KNN,KNND] = knnsearch(Xval,Xval,'K',k+1,'NSMethod','kdtree','Distance',disttype);
    KNN = uint32(KNN);   
    clear XVal;
    ...
    KNND(:,end+1:end+k) = 0; % adicionando mais k espaços para vizinhança recíproca
    ...
    for i=1:qtnodeval        
        ...
        KNND(sub2ind(size(KNN),KNN(i,1:k),(knns(KNN(i,1:k))+1)'))=KNND(i,1:k); % adicionando peso na ligação recíproca
        ...
        if max(knns)==size(KNN,2) % se algum nó atingiu o limite de colunas da matriz de vizinhança recíproca teremos de aumentá-la
            ...
            KNND(:,max(knns)+1:round(max(knns)*1.1)+1) = zeros(qtnodeval,round(max(knns)*0.1)+1);
        end
    end
    % removendo duplicatas
    for i=1:qtnodeval
        [knnrow, knnrowind] = unique(KNN(i,:),'stable'); % remove as duplicatas
        ...
        KNND(i,1:knns(i)) = KNND(i,knnrowind(1:end-1));
    end
    clear knnrow;
    KNN = KNN(:,1:max(knns)); % eliminando colunas que não tem vizinhos válidos
    KNND = KNND(:,1:max(knns)); % eliminando colunas que não tem vizinhos válidos
    ...
    strwalk37loop(maxiter, npart, nclass, stopmax, pgrd, dexp, deltav, deltap, potmin, partnode, partclass, potpart, slabelval, knns, distnode, KNN, KNND, potval, owndeg);
    clear KNN KNND distnode slabelval;
