...
% p_grd = 1 - potpart (v.31)
% Usage: [owner, pot, owndeg, distnode] = strwalk8k(X, slabel, k, disttype, valpha, deltav, deltap, dexp, nclass, maxiter)
% INPUT:
...
function [owner, pot, owndeg, distnode] = strwalk31(X, slabel, k, disttype, valpha, deltav, deltap, dexp, nclass, maxiter)
    ...
    for i=1:maxiter
        ...
        disp(sprintf('Particulas fracas: %i',sum(potpart<0.5)))
        if mod(i,10)==0
