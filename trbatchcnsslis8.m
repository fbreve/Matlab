...
for i=i_start:1:50
    ...
    fitfuncnsslis = @(x)fitcnsslis8(x,img,imgslab,gt,disttype);
    ...
    owner = cnsslis8(img, imgslab, [], 0, disttype);
    ...
    % grave imagem
    imwrite(imgres,sprintf('res/imgcnsslis8-%s-%s-err%0.4f-k%i.png',imgname,getenv('computername'),y,k));
    dlmwrite(sprintf('res/imgcnsslis8-%s-%s-err%0.4f-k%i.txt',imgname,getenv('computername'),y,k),[y telapsed k]);
    save(sprintf('tabs_cnsslis8-%s',getenv('computername')),'tab_k','tab_y','tab_time');
end;