...
% Usage: [indint,KNN,knns,X] = strwalk32indint(img, imgslab, k, fw, expo, disttype)
% INPUT:
...
% expo      - exponential to apply to the index
% disttype  - use 'euclidean', 'seuclidean', etc.
% OUTPUT:
% indint    - index of intraclass edges / total edges among a pair of 
%             labeled nodes [0 1]
...
function [indint,KNN,knns,X] = strwalk32indint2(img, imgslab, k, fw, expo, disttype)
    ...
    indint = (intrac / (intrac+interc)).^expo;
end
