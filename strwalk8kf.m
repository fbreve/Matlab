% Semi-Supervised Territory Mark Walk v.8kef
...
% Usage: [owner, pot, owndeg, distnode] = strwalk8kf(X, slabel, k, disttype, pdet, deltav, deltap, dexp, nclass, iter)
function [owner, pot, owndeg, distnode] = strwalk8kf(X, slabel, k, disttype, pdet, deltav, deltap, dexp, nclass, iter)
    ...
    npart = single(sum(slabel~=0)); % quantidade de partículas
    stopmax = round(qtnode/npart); % qtde de iterações para verificar convergência
...
%     BS = B(:,k+1);
%     clear B;
%     graph = sparse(W <= repmat(BS,1,qtnode));  % conectando k-vizinhos mais próximos
%     clear BS W;
%     graph = graph | graph';
%     graph = graph - speye(qtnode);  % zerando diagonal do grafo
    %disp('Rede montada');
    %graph = X;
