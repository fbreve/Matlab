...
for i=i_start:1:50
    ...
    fitfuncnsslis = @(x)fitcnsslis11(x,img,imgslab,gt,fwtype);
    ...
    % grave imagem
    imwrite(imgres,sprintf('res/imgcnsslis11-%s-%s-err%0.4f-k%i.png',imgname,getenv('computername'),y,k));
    dlmwrite(sprintf('res/imgcnsslis11-%s-%s-err%0.4f-k%i.txt',imgname,getenv('computername'),y,k),[y telapsed k]);
    save(sprintf('res/tabs_cnsslis11-%s',getenv('computername')),'tab_k','tab_y','tab_time');
end