...
for i=i_start:1:50
    ...
    err = zeros(201,1);
    parfor ki=1:201
        owner = cnsslis9teste(img, imgslab, fw, ki-1, sigma);
    ...
    tstart = tic;
    owner = cnsslis9teste(img, imgslab, fw, k, sigma);
    telapsed = toc(tstart);
    tab_time(i) = telapsed;
    imgres = own2img(owner,img,0);
    ...
    % grave imagem
    imwrite(imgres,sprintf('res/imgcnsslis9teste-%s-%s-err%0.4f-k%i.png',imgname,getenv('computername'),y,k));
    dlmwrite(sprintf('res/imgcnsslis9teste-%s-%s-err%0.4f-k%i.txt',imgname,getenv('computername'),y,k),[y telapsed k]);
