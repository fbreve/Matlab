...
for i=i_start:1:49
    ...
    fprintf('FINAL: Erro: %0.4f  Tempo: %8.2f  K: %4.0f\n',y,telapsed,k);
    % grave imagem
    imwrite(imgres,sprintf('res/imgstrwalk36gsc-%s-%s-err%0.4f-k%i.png',imgname,getenv('computername'),y,k));
    dlmwrite(sprintf('res/imgstrwalk36gsc-%s-%s-err%0.4f-k%i.txt',imgname,getenv('computername'),y,k),[y telapsed k]);
    save(sprintf('tabs_strwalk36feat-%s',getenv('computername')),'tab_k','tab_y','tab_time');
end