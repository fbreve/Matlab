...
tab_acc = zeros(kmax,amwrlabmax);
tab_kap = zeros(kmax,amwrlabmax);
...
for i=13:1:kmax
    ...
    for j=1:1:amwrlabmax
        ...
        amwrlab = j*0.02 - 0.02;
        parfor l=1:rep
            [graph, label] = graphgen3([128 128 128 128],i*2,k);
            ...
            disp(sprintf('K: %2.0f Rótulo Errado: %0.2f Iteração: %3.0f Acerto: %0.4f Kappa: %0.4f',k,amwrlab,l,iter_acc(l),iter_kap(l)))
        end
        ...
        disp(sprintf('MEDIA: K: %0.2f Rótulo Errado: %0.2f Acerto: %0.4f Kappa: %0.4f',k,amwrlab,tab_acc(i,j),tab_kap(i,j)))
        %save tab_wlzout tab_acc tab_kap tab_acc2 tab_kap2
        save tab_wlk tab_acc tab_kap
    end   
end