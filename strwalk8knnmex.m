% Semi-Supervised Territory Mark Walk v.8knn
...
% Recebe diretamente uma lista de vizinhos (v.8knn)
% Usage: [owner, pot, owndeg] = strwalk8knnmex(KNN, slabel, valpha, pgrd, deltav, deltap, dexp, nclass, maxiter)
% INPUT:
% KNN       - Matrix where each line is a data item and each column is a k-nearest neighbor
...
function [owner, pot, owndeg] = strwalk8knnmex(KNN, slabel, valpha, pgrd, deltav, deltap, dexp, nclass, maxiter)
    ...
    qtnode = size(KNN,1); % quantidade de nós    
    % constantes
    ...
    KNNR = zeros(qtnode,size(KNN,2)); % criando matriz para vizinhança recíproca, inicialmente com tamanho k
    ...
    [pot, owndeg] = strwalk8kloop(maxiter, npart, nclass, stopmax, pgrd, dexp, deltav, deltap, potmin, partpos, partclass, potpart, slabel, knns, distnode, KNN, pot, owndeg);
    [~,owner] = max(pot,[],2);
